...
Description:
    Copy a marker and create a new name. It can be useful to have duplicates to define the same marker in different rigid bodies.

Usage:
    python copy_marker.py -i inputFile.c3d/trc -o outputFile.c3d/trc -m1 "marker1" -nm "newMarkerName"
    or import as module
...
text="copy_marker module"

def copy_marker(acq,args):
    
    import numpy as np

    # Create an object (point) for each marker
    point1 = acq.GetPoint(args.marker1)
    #point2 = acq.GetPoint(args.newMarkerName)
    ...
    newpoint.SetValues(point1.GetValues()[:]) # set the value
...
if __name__ == '__main__':
    ...
    ######################################################
    acq_modified = copy_marker(acq,args)
